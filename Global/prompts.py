...
def json_split_prompt(
    ...
    key: str,
    format: str = """
    注意b与e里面的信息是帮助段落文本定位的, 需要具有唯一性, 并且要是处理的文本中的段落内容, 不可以杜撰, 保持原样输出
    解释"合并来源", 就是划分的段落使用了多个content的内容, 来保证语义分割的连续性
    如果使用来多个来content, 只需要合并他们的文件名, 来源, 日期为为一个json数组即可
    json格式为
    {
        "length": 2757,
        "data": [
    ...
    ChatPromptT = ChatPromptTemplate.from_messages(
        [
            ...
            (
                "user",
                """
                按以下步骤处理json里的文本任务，json里的每个上下数组元素的"context"是连续的文本
                1. 按照标点符号、文本语义划分长度为{length}的文本划分大小你自己确定。
                2. 提出每个文本的:压缩语义与原始文本（压缩语义保留关键字: {key}）。
                3. 按照指定的json格式输出，如果语义完整合并多个来源是允许的: {format}  
                被处理的json:{json} 
             """,
            ),
        ]
    ...
    return ChatPromptT.invoke(
        {"json": json, "length": length, "key": key, "format": format}
    )
